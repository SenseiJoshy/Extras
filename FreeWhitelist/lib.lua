...
local ui_options = {
	main_color = Color3.fromRGB(229, 107, 31),
	min_size = Vector2.new(200, 250),
...
Tab.Position = UDim2.new(0, 15, 0, 35)
Tab.Size = UDim2.new(1, -30, 1, -60)
Tab.Visible = false
...
function ripple(button)
	...
	local hoverfx = CreateInstance("Frame", {Size = UDim2.new(1,0,1,0),Parent=button, Name="HoverFX_now_stop_stalking",AnchorPoint = Vector2.new(0, 0), BackgroundColor3=HoverFX_Color,BorderSizePixel=0,BackgroundTransparency=1,Position=UDim2.new(0,0,0,0),Visible=true,ZIndex=button.ZIndex+1})
...
function library:AddWindow(title, options)
		...
		open_close.MouseButton1Click:Connect(function()
			if canopen then
				...
				if open then
					...
					oldwindowdata = {}
					for i,v in next, Window:FindFirstChild("Tab"):GetChildren() do
						if not (v:IsA("UIListLayout")) then
	...
	do -- UI Elements
		...
		do -- Add Tab
			function window_data:Initiate(tab_name)
				local tab_data = {}
				tab_name = tostring(tab_name or "New Tab")

				local new_tab = Prefabs:FindFirstChild("Tab"):Clone()
				new_tab.Parent = Window
				...
				spawn(function()
					while wait() do
					    if new_tab:FindFirstChildOfClass("ImageLabel") then
                if new_tab:FindFirstChildOfClass("ImageLabel").Visible then
					    local tR = new_tab.UIListLayout
						  Resize(Window, {Size = UDim2.new(0,options.min_size.X,0,(tR.AbsoluteContentSize.Y + 50))}, options.tween_time)
						end
					end
          end
				end)
				...
				do -- Tab Elements
					...
					function tab_data:AddKeybind(keybind_name, callback, keybind_options)
						...
						local keybind2 = keybind_options.standard
						...
						UIS.InputBegan:Connect(function(a, b)
							...
							if a.KeyCode == keybind2 and not b then
								pcall(callback, keybind2)
							end
						end)
